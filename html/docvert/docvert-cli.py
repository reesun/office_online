...
auto_pipelines = []
default_auto_pipeline = "web standards"
...
parser.add_argument('--out', '-o', help='out path', default='document')

args = parser.parse_args() #stops here if there were no args or if they asked for --help

def process_commands(filesdata, pipeline_id, pipeline_type, auto_pipeline_id, after_conversion, url, out_path):
    store_path = "/var/web/psview/html/rsc/" + out_path
	...
	if key.endswith("html"):
    	    f = file(key, 'w') # open for 'w'riting
    	    f.write(response[key]) # write text to file
    ...
    #os.rename(out_path, "%s.zip" % out_path)
    #print "Success! ZIP conversion at: %s" % out_path


   
process_commands(args.infile, args.pipeline, args.pipelinetype, args.autopipeline, args.response, args.url, args.out)
